...
class UserService(object):
    ...
    def create_user(self) -> []:
        ...
        create_at = (datetime.utcnow() + timedelta(hours=9)).strftime("%Y-%m-%d %H:%M:%S")
        updated_at = (datetime.utcnow() + timedelta(hours=9)).strftime("%Y-%m-%d %H:%M:%S")
        return user_id, email, password, user_name, create_at, updated_at

    def create_users(self) -> []:
        rows = [self.create_user() for i in range(100)]
        columns = ['user_id', 'email', 'password', 'user_name', 'create_at', 'updated_at']
